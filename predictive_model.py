...
RESULTS_FILE = 'projects/script_result/stage1/stage1_grid_search_WH_results.csv' # Results file for Stage 1 Grid Search
DEFAULT_MODEL_SAVE_PATH = 'projects/script_result/stage1/predictive_model_xgb_s1.joblib' 
DEFAULT_FEATURE_COLS_SAVE_PATH = 'projects/script_result/stage1/feature_columns_xgb_s1.joblib' 
STAGE1_PREDICTIONS_OUTPUT_FILE = 'projects/script_result/stage1/stage1_predictions.csv' # Output for Stage 2
...
def ensure_dir(file_path):
    ...
    if directory and not os.path.exists(directory): 
        try: 
             print(f"Creating directory: {directory}", flush=True)
             os.makedirs(directory, exist_ok=True)
        except OSError as e: print(f"Warn: Could not create dir {directory}: {e}", flush=True)
...
def load_data(sensor_file=SENSOR_FILE, failure_file=FAILURE_FILE, 
    ...
    try:
        ...
        else: print(f"Warn: Failure log '{failure_file}' empty.", flush=True); loaded_data['failure'] = pd.DataFrame(columns=['Machine_ID', 'Timestamp', 'Failure_Type']); loaded_data['failure']['Timestamp'] = pd.to_datetime(loaded_data['failure']['Timestamp'])
        if os.path.exists(equipment_file) and os.path.getsize(equipment_file) > 0: loaded_data['equipment']=pd.read_csv(equipment_file)
        else: print(f"Warn: Equipment file '{equipment_file}' empty.", flush=True); loaded_data['equipment'] = pd.DataFrame(columns=['Machine_ID', 'Equipment_Age (Years)', 'Usage_Cycles'])
        ...
        else: print(f"Warn: Maintenance log '{maintenance_file}' empty.", flush=True); loaded_data['maintenance'] = pd.DataFrame(columns=['Machine_ID', 'Timestamp', 'Maintenance_Action']); loaded_data['maintenance']['Timestamp'] = pd.to_datetime(loaded_data['maintenance']['Timestamp'])
        print(f"Loaded sensor: {loaded_data['sensor'].shape}", flush=True); print(f"Loaded failure: {loaded_data['failure'].shape}", flush=True); print(f"Loaded equipment: {loaded_data['equipment'].shape}", flush=True); print(f"Loaded maintenance: {loaded_data['maintenance'].shape}", flush=True)
        return loaded_data['sensor'], loaded_data['failure'], loaded_data['equipment'], loaded_data['maintenance']
    except FileNotFoundError as e: print(f"Error loading file: {e}.", flush=True); return None, None, None, None
    except Exception as e: print(f"Error loading data: {e}", flush=True); return None, None, None, None
...
def create_features(df, window_size_hrs, sensor_cols=['Temperature', 'Vibration', 'Pressure', 'Current', 'AFR', 'RPM']):
    ...
    print(f"\n--- Creating Features (W={window_size_hrs}h, No FFT) ---", flush=True)
    if not isinstance(df.index, pd.DatetimeIndex): df = df.set_index('Timestamp').sort_index()
    if df.empty: print("Error: Input empty.", flush=True); return pd.DataFrame() 
    ...
    if freq is None and len(df_out.index) > 1: 
        print("Attempting manual frequency inference from median difference...", flush=True)
                ...
                if len(positive_diffs) > 0:
                    ...
                    if median_diff_ns > 0:
                        ...
                        if freq_offset: 
                            ...
                            print(f"Inferred frequency manually: {freq}", flush=True)
                        else:
                            print("Warn: Manual inference - could not convert timedelta to offset.", flush=True)
                    else:
                        print("Warn: Manual inference - median difference is non-positive.", flush=True)
                else:
                    print("Warn: Manual inference - no positive time differences found.", flush=True)
            else:
                print("Warn: Manual inference - not enough unique timestamps.", flush=True)                 
        # --- CORRECTED INDENTATION for except ---
        except Exception as e: 
            print(f"Warn: Manual frequency inference failed with error: {e}", flush=True)          
    ...
    except Exception as e: print(f"Error calc periods: {e}"); window_periods = int(timedelta(hours=window_size_hrs) / timedelta(seconds=1800), flush=True); print(f"Using default periods: {window_periods}", flush=True)
    if window_periods <= 0: window_periods = 2;
    if window_periods < 2 : print(f"Warn: Window periods {window_periods}<2.", flush=True); window_periods = 2
    ...
    print(f"Using window periods: {window_periods}", flush=True)
    
    # --- Feature Calculation ---
    print("Calculating rolling features...", flush=True)
    ...
    if not isinstance(df_out_grouped.index, pd.DatetimeIndex):
         ...
         else: print("Error: Timestamp index missing.", flush=True); return pd.DataFrame()
    ...
    for col in sensor_cols:
        ...
        print(f"  Calculating features for: {col}...", flush=True) 
    ...
    print("  Calculating derived features...", flush=True)
    for col in sensor_cols:
         if col not in df_out.columns: continue
    ...
    if 'Vibration' in sensor_cols and 'Current' in sensor_cols: # Correlation
        print("  Calculating correlation...", flush=True)
    ...
    print(f"\n  Feature calculation complete. Time: {(datetime.now() - start_time_feat).total_seconds():.2f}s", flush=True)
    print("  Handling NaNs...", flush=True); df_out = df_out.fillna(method='bfill').fillna(0); 
    ...
    if valid_static_cols: 
        print("  Adding static features...", flush=True); 
        try: 
            ...
            if df_out.index.is_unique and 'Machine_ID' in df_out.columns:
                 ...
                 print("    Using merge for static features due to index/column state...", flush=True)
            ...
            print(f"Static features added. Shape: {df_out.shape}", flush=True)
        except Exception as e: print(f"Warning: Error adding static features: {e}", flush=True)
    else: print("Warn: Static features not found.", flush=True)
    
    print("Feature creation complete.", flush=True)
    return df_out
    ...
    if valid_static_cols: 
        ...
        try: 
            ...
            if 'Machine_ID' in df_out_to_merge.columns:
                ...
                # Set index back
                if 'Timestamp' in df_out_merged.columns: df_out = df_out_merged.set_index('Timestamp').sort_index()
                else: df_out = df_out_merged; print("Warn: Timestamp missing after static merge.", flush=True)
                ...
                if not df_out.index.equals(original_index) and isinstance(original_index, pd.DatetimeIndex):
                     print("Warning: Index changed during static feature merge. Attempting reindex.", flush=True)
                     # Try reindexing back (might fail if duplicates were introduced/handled differently)
                     try: df_out = df_out.reindex(original_index)
                     except Exception as reindex_e: print(f"Reindex failed: {reindex_e}", flush=True)
                print(f"Static features added. Shape: {df_out.shape}", flush=True)
            else: print("Warn: Machine_ID missing for static merge.", flush=True)
        except Exception as e: print(f"Warn: Error adding static features: {e}", flush=True)
    else: print("Warn: Static features not found in original df.", flush=True)
...
def create_target(df, failure_df, prediction_horizon_hrs, target_col_name='Failure_Within_H', roll_start_ts=None, roll_end_ts=None):
    ...
    print(f"\n--- Creating Target Variable '{target_col_name}' (H={prediction_horizon_hrs}h) ---", flush=True)
    if not isinstance(df.index, pd.DatetimeIndex): print("Error: df index must be DatetimeIndex.", flush=True); return df
    df_out = df.copy(); 
    if failure_df is None or failure_df.empty: print("Warning: Failure log empty.", flush=True); df_out[target_col_name] = 0; return df_out
    if not df_out.index.is_monotonic_increasing: print("Warning: Sorting index.", flush=True); df_out = df_out.sort_index()
    ...
    print("  Assigning target labels...", flush=True); total_rows = len(df_out); processed_rows = 0; start_target_time = datetime.now()
    try: machine_id_array = df_out['Machine_ID'].values
    except KeyError: print("Error: 'Machine_ID' missing.", flush=True); return df_out
    ...
    print(f"\n    Finished target loop. Time: {(datetime.now() - start_target_time).total_seconds():.2f}s", flush=True)
    ...
    print("Target created.", flush=True); print(f"Target proportion:\n{df_out[target_col_name].value_counts(normalize=True)}", flush=True)
    return df_out
...
def train_xgboost_model(features_df, target_col='Failure_Within_H', test_size=DEFAULT_TEST_SET_SIZE, enable_plotting=False, model_path=None, feature_cols_path=None): 
    ...
    print(f"\n--- Training Model (XGBoost v1.x, Target: {target_col}) ---", flush=True)
    ...
    if not features_df.empty: # Feature Cleaning
         ...
         for col in temp_X.columns:
             if temp_X[col].dtype == 'object': # Exclude non-numeric objects
                  ...
                  except ValueError: print(f"  Excluding object: {col}", flush=True); continue 
             if temp_X[col].nunique(dropna=False) < 2 or temp_X[col].isnull().all(): print(f"  Excluding constant/NaN: {col}", flush=True); continue # Exclude constant/NaN
             valid_cols.append(col)
    feature_cols = valid_cols
    if not feature_cols: print("Error: No valid features.", flush=True); return model, feature_cols, X_test, y_test, y_pred_proba, machine_ids_test, roc_auc, pr_auc
    ...
    y_train, y_test = y.iloc[:split_point], y.iloc[split_point:]; print(f"Train/Test sizes: {len(X_train)} / {len(X_test)}", flush=True)
    train_counts = y_train.value_counts(); test_counts = y_test.value_counts(); 
    if len(train_counts) < 2: print("Error: Only one class in train target.", flush=True); return model, feature_cols, X_test, y_test, y_pred_proba, machine_ids_test, roc_auc, pr_auc
    ...
    if 'Machine_ID' in features_df.columns:
         ...
         except Exception as e: print(f"Error getting Machine_IDs: {e}", flush=True)
    print("\nTraining XGBoost...", flush=True); neg_count = train_counts.get(0, 0); pos_count = train_counts.get(1, 0)
    scale_pos_weight = neg_count / pos_count if pos_count > 0 else 1; print(f"Scale_pos_weight: {scale_pos_weight:.2f}", flush=True)
    ...
    except TypeError: print(f"Fit error. Retrying without early stopping.", flush=True); model.fit(X_train, y_train, verbose=False) 
    except Exception as e: print(f"Fit error: {e}", flush=True); return model, feature_cols, X_test, y_test, y_pred_proba, machine_ids_test, roc_auc, pr_auc
    print(f"Training complete. Time: {(datetime.now() - start_train_time).total_seconds():.2f}s", flush=True)
    best_iteration = model.best_iteration if hasattr(model, 'best_iteration') and model.best_iteration > 0 else model.n_estimators
    print(f"Best iteration: {best_iteration if hasattr(model, 'best_iteration') and model.best_iteration > 0 else 'N/A (Final)'}", flush=True)
    print("\n--- Evaluating Model ---", flush=True); 
    try: y_pred_proba = model.predict_proba(X_test)[:, 1] 
    except Exception as e: print(f"Predict_proba error: {e}.", flush=True); return model, feature_cols, X_test, y_test, y_pred_proba, machine_ids_test, roc_auc, pr_auc
    try: roc_auc = roc_auc_score(y_test, y_pred_proba)
    except Exception: print("ROC AUC calc error", flush=True)
    try: precision, recall, _ = precision_recall_curve(y_test, y_pred_proba); pr_auc = auc(recall, precision)
    except Exception: print("PR AUC calc error", flush=True)
    print(f"Overall ROC AUC Score: {roc_auc:.4f}", flush=True); print(f"Overall PR AUC Score: {pr_auc:.4f}", flush=True)
    if enable_plotting: # Plotting logic...
        ...
        try: # Feature Importance ...
             ...
             print("\nTop 15 Features (Gain):", flush=True); print(imp_series.head(15), flush=True) 
        except Exception as e: print(f"Feature Importance Plot Error: {e}", flush=True)
    ...
    if model_path and feature_cols_path:
        try:
            ...
            print(f"\nModel saved: {model_path}"); print(f"Features saved: {feature_cols_path}", flush=True)
...
def apply_bayesian_filter(df_test_results, proba_col_name='xgb_proba', 
    ...
    print(f"\n--- Applying Bayesian Filter (alpha={belief_alpha}, th={belief_threshold}, steps={consecutive_steps}) ---", flush=True)
    ...
    for i, (idx, row) in enumerate(df_results.iterrows()): 
        if i % 100000 == 0: print(f"  Filter progress {i}/{n}...", end='\r', flush=True)
    ...
    print(f"\n  Filter done. Time: {(datetime.now() - start_bayes_time).total_seconds():.2f}s", flush=True) 
    ...
    print(f"Bayes Alarm dist:\n{df_results['bayesian_alarm'].value_counts()}", flush=True)
    return df_results
...
def save_results_to_csv(filepath, results_data):
    ...
    print(f"\nAttempting to write results to {filepath}...", flush=True)
    try:
        ...
        print("Results written successfully.", flush=True)
        
    except IOError as e: # Catch specific I/O errors
         print(f"I/O Error writing results to CSV: {e}", flush=True)
    except Exception as e: 
        print(f"General Error writing results to CSV: {e}", flush=True)
...
def run_pipeline(feature_window_hrs, prediction_horizon_hrs, 
    ...
    print("="*70, flush=True); print(f" Starting Run: W={feature_window_hrs}h, H={prediction_horizon_hrs}h ", flush=True); print("="*70, flush=True)
    ...
    try: # Wrap major steps in try-except
        ...
        # 2. Prepare Data
        print("Preparing data...", flush=True); merged_df = pd.DataFrame() 
        ...
        if merged_df.index.duplicated().any(): print("WARNING: Duplicates after merge!", flush=True)
        ...
        print(f"Shape before final dedupe: {final_df.shape}", flush=True)
        final_df = final_df[~final_df.index.duplicated(keep='first')]
        print(f"Shape after final dedupe: {final_df.shape}", flush=True)
        ...
        else: print("\nSkipping model training.", flush=True); model_trained_successfully = False
        ...
        if model_trained_successfully: 
            print("\n--- Detailed Threshold Analysis (Stage 1) ---", flush=True)
            try: # F1 Optimization... 
                ...
                if len(f1_scores_aligned) > 0 and len(thresholds_pr) == len(f1_scores_aligned):
                    ...
                    print(f"Optimal Threshold (Max F1): {optimal_threshold_f1:.4f} -> F1={max_f1_score:.4f} (P={precision_at_max_f1:.4f}, R={recall_at_max_f1:.4f})", flush=True)
            ...
            if X_test is not None and y_test is not None and machine_ids_test is not None:
                 print("\nApplying Bayesian filter (Stage 1 Eval)...", flush=True)
                 try: # Bayes logic... 
                     ...
                     if not test_results_with_bayes_s1.empty:
                          ...
                          print(f"Bayesian Metrics (S1): P={bayes_precision:.3f}, R={bayes_recall:.3f}, F1={bayes_f1:.3f}", flush=True)
                 ...
                 except Exception as e: print(f"Error during Bayesian analysis: {e}", flush=True)
            else: print("Skipping Bayesian filter: Missing data.", flush=True)
                     
            # --- Save Stage 1 Predictions ---
            if y_pred_proba_test is not None and machine_ids_test is not None and y_test is not None:
                 print(f"\n--- Saving Stage 1 Predictions to {stage1_pred_output_path} ---", flush=True)
                 try: # Save S1 preds logic... 
                      ...
                      print("S1 predictions saved.", flush=True)
    ...
    print(f"\n--- Finished Run: W={feature_window_hrs}h, H={prediction_horizon_hrs}h ---", flush=True)
    return results 
...
if __name__ == "__main__":
    ...
    try:
        ...
        if pipeline_results: # Check if run_pipeline returned results
            ...
            if 'save_results_to_csv' in globals():
                ...
                print("Save function missing, cannot save results.", flush=True)
        else: # Handle case where run_pipeline might return None on failure
            print("Pipeline did not return results.", flush=True)
    ...
    except SystemExit: print("Pipeline exited.", flush=True) 
    except Exception as e: 
        print(f"Critical error during pipeline execution: {e}", flush=True)
        ...
        else: print("Could not save error results.", flush=True)
    # --- REMOVED finally block with save_results_to_csv ---
    
    print("\n--- predictive_model.py Script Finished ---", flush=True)
...
def save_results_to_csv(filepath, results_data):
    ...
    # ...(Same code as before)...
    print(f"Writing results to {filepath}...", flush=True)
    try:
        ensure_dir(filepath) 
