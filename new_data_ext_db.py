...
def clamp_integer(value):
    """Ensure integer values are within PostgreSQL INTEGER type limits and handle NaN values"""
    if pd.isna(value):
        return None  # Convert NaN to NULL for database insertion
    max_int = 2147483647
    min_int = -2147483647
    return max(min(int(value), max_int), min_int)

def insert_sensor_data_to_db(sensor_df):
    """Insert sensor data into the prediction_data table"""
    conn = None
    try:
        ...
        sensor_data = sensor_df.reset_index()
        
        print(f"Preparing {len(sensor_data)} records for database insertion...")
        tuples = [
        ...
        print("Inserting data into prediction_data table...")
        ...
        conn.commit()
        print(f"Successfully inserted {len(tuples)} sensor data records into prediction_data table")
    except psycopg2.Error as e:
        if conn:
            conn.rollback()
...
# Updated date range to start from 2021-01-01 and end at current date
...
FAILURE_POTENTIAL_WEEK_INTERVAL = 3       # Potential arises every 3 weeks
...
def generate_base_sensor_data(start_date_str, end_date_str, freq, machine_ids):
    ...
    for machine in machine_ids:
        for timestamp in date_range:
            ...
            sensor_data.append([timestamp, machine, afr, current, pressure, clamp_integer(rpm), temperature, vibration])
            ...
            if idx >= effective_trend_start_time:
                ...
                for sensor, max_delta in config['sensors'].items():
                    ...
                    if sensor == 'RPM_std_factor':
                         ...
                         sensor_df_with_trends.loc[idx, 'RPM'] = clamp_integer(round(new_rpm))
...
if __name__ == "__main__":
    print(f"--- Starting Data Generation for PostgreSQL Database ({START_DATE} to {END_DATE}) ---")
    ...
    # 8. Insert data into PostgreSQL database
    print("\nInserting data into PostgreSQL database...")
    try:
        insert_sensor_data_to_db(final_sensor_df)
        print("\n--- Data Generation Complete. Data inserted into prediction_data table. ---")
    except Exception as e:
        print(f"\nError: Failed to insert data into database: {e}")
        print("\nFalling back to saving CSV files...")
        # Create data directory if it doesn't exist
        os.makedirs('data/backup', exist_ok=True)
        backup_timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        final_sensor_df.reset_index().to_csv(f'data/backup/sensor_data_{backup_timestamp}.csv', index=False)
        print(f"Data saved to CSV backup file: data/backup/sensor_data_{backup_timestamp}.csv")
